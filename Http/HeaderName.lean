...
This file defines the `HeaderName` type, which represents the name (key)
of an HTTP header.
-/
...
/- metaprogram for generating common header names -/
section
open Lean Macro Elab Qq Command Meta Parser

scoped elab "genStandardHeaders!" names:term : command => do
  let expectedType := q(Array (Name × String × String))
  let list ← liftTermElabM do
    let elabList ← instantiateMVars <| ← Term.withSynthesize <|
          Term.elabTermEnsuringType names (expectedType? := some expectedType)
    return ← unsafe evalExpr (Array (Name × String × String)) expectedType elabList
  ...
  elabCommand (← `(
    ...
    inductive $indName
      $[ $docstrings:docComment | $ctors:ident ]*
    deriving DecidableEq, Hashable, Ord, Repr

    namespace $indName

    /-- Get the (lowercase) header string for the standard header. -/
    def $(mkIdent `toHeaderString) : $indName → String
      $[ | $ctors:ident => $( list.map (Syntax.mkStrLit ·.2.1) ):term ]*

    /-- List of all standard headers. -/
    def $(mkIdent `all) : List $indName :=
      $( ← ctors.foldrM (fun a b => `($a:ident :: $b:term)) (← `([])) )
...
| custom : CaseInsString → HeaderName
deriving DecidableEq, Hashable, Repr

namespace HeaderName

def toHeaderString : HeaderName → String
| standard s => s.toHeaderString
| custom s => s.toLower

def ofHeaderString (s : String) :=
  let s := s.toLower
  match Standard.parse s with
  | some n => standard n
